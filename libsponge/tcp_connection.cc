...
size_t TCPConnection::write(const string &data) {
    ...
    size_t actual_writen_bytes = _sender.stream_in().write(data);
    _sender.fill_window();
    send_sender_segments();
    return actual_writen_bytes;
}
...
void TCPConnection::clean_shutdown() {
    if (_receiver.stream_out().input_ended()) {
        if (!_sender.stream_in().eof()) {
            ...
            if (_linger_after_streams_finish == true && 
                time_since_last_segment_received() >= 10 * _cfg.rt_timeout) {
                _active = false;
            } else if (_linger_after_streams_finish == false) {
                _active = false;
            } 
        }    
    }
